...
class Login extends Action
{
   ...
   protected $storeManager;
    ...
    public function execute(){
        $foursquare = $this->foursquare->newLive();
        $value = $foursquare->authenticate(self::SOCIAL_TYPE);
        $user_profile = $value->getUserProfile();
        $email = $user_profile->email;
        if($email==NULL){
           $this->messageManager->addError('Email is Null, Please enter email in your Foursquare profile');
        ...
        if ($customer) {
            ...
            $this->helperData->setAuthorCustomer($user_profile->identifier, $customer->getId(),self::SOCIAL_TYPE,$this->helperFoursquare->sendPassword());
             $this->session->setCustomerAsLoggedIn($customer);
