...
class Login extends Action
{
    ...
    {
        $google = $this->google->newLive();
        $value = $google->authenticate(self::SOCIAL_TYPE);
        $user_profile = $value->getUserProfile();
         //$email = $name . '@google.com';
        $email = $user_profile->email;
        if($email==NULL){
           $this->messageManager->addError('Email is Null, Please enter email in your Google profile');
        ...
        if ($customer) {
            ...
            $this->helperData->setAuthorCustomer($user_profile->identifier, $customer->getId(),self::SOCIAL_TYPE,$this->helperGoogle->sendPassword());
             $this->session->setCustomerAsLoggedIn($customer);
