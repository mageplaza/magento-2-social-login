...
use Mageplaza\SocialLogin\Model\Github;
use Magento\Store\Model\StoreManagerInterface;
use Mageplaza\SocialLogin\Helper\Github\Data as HelperGithub;
...
class Login extends Action
{
	...
	public function __construct(
		...
		$this->helperGithub   = $helperGithub;
	...
	{
		$github = $this->github->newLive();
		 $value = $github->authenticate(self::SOCIAL_TYPE);
		 $user_profile = $value->getUserProfile();
		 $name = $user_profile->displayName;
		 $email = $user_profile->email;
		 if($email==NULL){
             $this->messageManager->addError('Email is Null, Please enter email in your Github profile');
         ...
         if ($customer) {
             ...
             if (!$customer || !$customer->getId()) {
                 ...
                 if ($this->helperGithub->sendPassword()) {
                         try {
             ...
             $this->helperData->setAuthorCustomer($user_profile->identifier, $customer->getId(),self::SOCIAL_TYPE,$this->helperGithub->sendPassword());
              $this->session->setCustomerAsLoggedIn($customer);
