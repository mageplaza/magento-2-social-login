...
class Facebook extends ProviderModel {
    ...
    function loginFinish() {
        ...
        if (!isset($accessToken)) {
            if ($helper->getError()) {
                throw new Exception(sprintf("Could not authorize user, reason: %s (%d)", $helper->getErrorDescription(), $helper->getErrorCode()));
            } else {
                throw new Exception("Could not authorize user. Bad request");
            }
        }
